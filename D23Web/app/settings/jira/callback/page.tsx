...
function JiraCallbackContent() {
  ...
  const [message, setMessage] = useState("Completing Jira connection...");

  useEffect(() => {
    const code = params.get("code");
    if (!code) {
      setStatus("error");
      setMessage("Missing authorization code. Try connecting Jira again.");
      return;
    }
    ...
    const completeConnection = async () => {
      try {
        const response = await authFetch(`${apiBase}/auth/providers/jira/exchange`, {
          method: "POST",
          body: JSON.stringify({ code, state }),
        });

        if (!response.ok) {
          let detail = "Failed to complete Jira connection.";
          try {
        ...
        setStatus("success");
        setMessage("Jira connected! Redirecting...");
        setTimeout(() => router.replace("/settings"), 1200);
      } catch (error) {
        setStatus("error");
        setMessage((error as Error).message || "Failed to connect Jira. Please try again.");
      }
    };
    ...
    <div className="min-h-screen flex items-center justify-center bg-white px-4">
      <div className="max-w-md w-full bg-white border border-neutral-200 rounded-xl p-8 shadow-sm">
        <div className="flex flex-col items-center gap-4">
          ...
          <h2 className="text-xl font-semibold text-neutral-900">Jira Authorization</h2>
