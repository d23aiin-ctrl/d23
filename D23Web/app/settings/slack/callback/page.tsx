...
function SlackCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setStatus("error");
      setMessage("Missing authorization code. Try connecting Slack again.");
      return;
    }
    ...
    const completeConnection = async () => {
      try {
        ...
        if (!response.ok) {
          let detail = "Failed to complete Slack connection.";
          try {
        ...
        setStatus("success");
        setMessage("Slack connected! Redirecting...");
        setTimeout(() => router.replace("/settings"), 1200);
      } catch (error) {
        setStatus("error");
        setMessage((error as Error).message || "Failed to connect Slack. Please try again.");
      }
    };
    ...
    <div className="min-h-screen flex items-center justify-center bg-white px-4">
      <div className="max-w-md w-full bg-white border border-neutral-200 rounded-xl p-8 shadow-sm">
        <div className="flex flex-col items-center gap-4">
          ...
          <h2 className="text-xl font-semibold text-neutral-900">Slack Authorization</h2>
