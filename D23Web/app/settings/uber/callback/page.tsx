...
function UberCallbackContent() {
  const params = useSearchParams();
  const router = useRouter();
  const apiBase = process.env.NEXT_PUBLIC_API_URL || "/api";
  const [status, setStatus] = useState<Status>("pending");
  const [message, setMessage] = useState("Completing Uber connection...");

  useEffect(() => {
    const code = params.get("code");
    if (!code) {
      setStatus("error");
      setMessage("Missing authorization code. Try connecting Uber again.");
      return;
    }
    ...
    const completeConnection = async () => {
      try {
        const response = await authFetch(`${apiBase}/auth/providers/uber/exchange`, {
          method: "POST",
          body: JSON.stringify({ code, state }),
        });

        if (!response.ok) {
          let detail = "Failed to complete Uber connection.";
          try {
        ...
        setStatus("success");
        setMessage("Uber connected! Redirecting...");
        setTimeout(() => router.replace("/settings"), 1200);
      } catch (error) {
        setStatus("error");
        setMessage((error as Error).message || "Failed to connect Uber. Please try again.");
      }
    };
    ...
    <div className="min-h-screen flex items-center justify-center bg-white px-4">
      <div className="max-w-md w-full bg-white border border-neutral-200 rounded-xl p-8 shadow-sm">
        <div className="flex flex-col items-center gap-4">
          ...
          <h2 className="text-xl font-semibold text-neutral-900">Uber Authorization</h2>
