...
const weatherIcons: Record<string, React.ReactNode> = {
  ...
  cloudy: <Cloud className="h-16 w-16 text-neutral-400" />,
  partly: <Cloud className="h-16 w-16 text-neutral-300" />,
  ...
  default: <Cloud className="h-16 w-16 text-neutral-400" />,
};
...
function getBackgroundGradient(condition: string | undefined) {
  if (!condition) return "from-violet-500/20 via-indigo-500/10 to-transparent";
  const lower = condition.toLowerCase();
  ...
  if (lower.includes("cloud")) {
    return "from-neutral-300/20 via-neutral-200/10 to-transparent";
  }
  ...
  return (
    <Card className="bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
      <CardContent className="p-0">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-neutral-500 text-lg">C</span>
              </div>
              <p className="text-lg text-neutral-600 mt-1 capitalize">{data.condition}</p>
        ...
        {hasDetailedData && (
          <div className="grid grid-cols-3 gap-4 p-4 border-t border-neutral-200 bg-neutral-50">
            {data.humidity !== null && (
              <div className="text-center">
                <Droplets className="h-5 w-5 mx-auto text-blue-400 mb-1" />
                <p className="text-xs text-neutral-400">Humidity</p>
                <p className="text-sm font-medium text-neutral-900">{data.humidity}%</p>
              </div>
            )}
            {data.wind_speed !== undefined && (
              <div className="text-center">
                <Wind className="h-5 w-5 mx-auto text-neutral-400 mb-1" />
                <p className="text-xs text-neutral-400">Wind</p>
                <p className="text-sm font-medium text-neutral-900">{data.wind_speed}</p>
              </div>
            )}
            {data.visibility !== undefined && (
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-neutral-900">{data.visibility}</p>
              </div>
            )}
          </div>
        )}
