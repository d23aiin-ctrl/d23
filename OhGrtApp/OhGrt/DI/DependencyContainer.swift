...
final class DependencyContainer {
    ...
    private var gmailRepository: GmailRepositoryProtocol?
    ...
    private func setupRepositories() {
        ...
        gmailRepository = GmailRepository(apiClient: apiClient)
    }
    ...
    // Auth Use Cases
    func makeSignInWithGoogleUseCase() -> SignInWithGoogleUseCaseProtocol {
        SignInWithGoogleUseCase(authRepository: authRepository)
    }

    func makeSignInWithGmailUseCase() -> SignInWithGmailUseCaseProtocol {
        SignInWithGmailUseCase(authRepository: authRepository)
    }

    func makeContinueAsGuestUseCase() -> ContinueAsGuestUseCaseProtocol {
        ContinueAsGuestUseCase(authRepository: authRepository)
    }
    ...
    // Gmail Use Cases
    func makeSearchGmailUseCase() -> SearchGmailUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return SearchGmailUseCase(repository: gmailRepository)
    }

    func makeGetGmailMessageUseCase() -> GetGmailMessageUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return GetGmailMessageUseCase(repository: gmailRepository)
    }

    func makeSendGmailMessageUseCase() -> SendGmailMessageUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return SendGmailMessageUseCase(repository: gmailRepository)
    }

    func makeScheduleEmailUseCase() -> ScheduleEmailUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return ScheduleEmailUseCase(repository: gmailRepository)
    }

    func makeGetScheduledEmailsUseCase() -> GetScheduledEmailsUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return GetScheduledEmailsUseCase(repository: gmailRepository)
    }

    func makeUpdateScheduledEmailUseCase() -> UpdateScheduledEmailUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return UpdateScheduledEmailUseCase(repository: gmailRepository)
    }

    func makeDeleteScheduledEmailUseCase() -> DeleteScheduledEmailUseCaseProtocol {
        guard let gmailRepository = gmailRepository else {
            fatalError("GmailRepository not configured. Call configure(with:) first.")
        }
        return DeleteScheduledEmailUseCase(repository: gmailRepository)
    }
    ...
    func makeAuthViewModel() -> AuthViewModel {
        AuthViewModel(
            signInWithGoogleUseCase: makeSignInWithGoogleUseCase(),
            signInWithGmailUseCase: makeSignInWithGmailUseCase(),
            signOutUseCase: makeSignOutUseCase(),
            observeAuthStateUseCase: makeObserveAuthStateUseCase(),
            continueAsGuestUseCase: makeContinueAsGuestUseCase()
        )
    }
    ...
    func makeChatViewModel() -> ChatViewModel {
        ChatViewModel(
            ...
            getMessagesUseCase: makeGetMessagesUseCase(),
            getToolsUseCase: makeGetToolsUseCase(),
            getProvidersUseCase: makeGetProvidersUseCase(),
            createConversationUseCase: makeCreateConversationUseCase()
        )
    }

    func makeGmailViewModel() -> GmailViewModel {
        GmailViewModel(
            searchUseCase: makeSearchGmailUseCase(),
            getMessageUseCase: makeGetGmailMessageUseCase(),
            sendMessageUseCase: makeSendGmailMessageUseCase(),
            scheduleEmailUseCase: makeScheduleEmailUseCase()
        )
    }

    func makeScheduledEmailsViewModel() -> ScheduledEmailsViewModel {
        ScheduledEmailsViewModel(
            getScheduledEmailsUseCase: makeGetScheduledEmailsUseCase(),
            updateScheduledEmailUseCase: makeUpdateScheduledEmailUseCase(),
            deleteScheduledEmailUseCase: makeDeleteScheduledEmailUseCase()
        )
    }

    func makeConversationListViewModel() -> ConversationListViewModel {
        ConversationListViewModel(
            getConversationsUseCase: makeGetConversationsUseCase(),
            createConversationUseCase: makeCreateConversationUseCase(),
            deleteConversationUseCase: makeDeleteConversationUseCase()
        )
    }
}
