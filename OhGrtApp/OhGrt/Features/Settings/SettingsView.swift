...
struct SettingsView: View {
    ...
    @StateObject private var chatViewModel = DependencyContainer.shared.makeChatViewModel()
    ...
    @State private var showingScheduledEmails = false
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showingTools) {
                ToolPickerView(
                    availableTools: chatViewModel.availableTools,
                    selectedTools: $chatViewModel.selectedTools,
                    errorMessage: chatViewModel.toolsError
                ) {
                    await chatViewModel.loadTools()
                } onToggle: { toolId in
                    chatViewModel.toggleTool(toolId)
                }
                .task {
                    if chatViewModel.availableTools.isEmpty {
                        await chatViewModel.loadTools()
                    }
                }
            }
            .sheet(isPresented: $showingMCP) {
                MCPConfigView(
                    providers: Binding(
                        get: { chatViewModel.providers },
                        set: { _ in }
                    ),
                    providerError: $chatViewModel.providerError,
                    isLoading: chatViewModel.providersLoading,
                    onConnectProvider: { provider in
                        Task {
                            await chatViewModel.connectProvider(provider, secret: "", displayName: "", config: nil)
                        }
                    },
                    onDisconnectProvider: { provider in
                        Task {
                            await chatViewModel.disconnectProvider(provider)
                        }
                    },
                    onRefresh: {
                        await chatViewModel.loadProviders()
                    }
                )
                .task {
                    await chatViewModel.loadProviders()
            ...
            .sheet(isPresented: $showingScheduledEmails) {
                ScheduledEmailsView()
            }
        }
        .onAppear {
    ...
    private var accountSection: some View {
            ...
            VStack(spacing: 12) {
                ...
                Button {
                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(authViewModel.isAuthenticated ? "Gmail Connected" : "Connect Gmail")
                                .font(.subheadline)
                                .fontWeight(.medium)
                                .foregroundColor(.primary)
                            Text(authViewModel.isAuthenticated ? "Signed in with Google" : "Sign in to use Gmail features")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
        ...
        .alert("Gmail Sign-In", isPresented: $authViewModel.showError) {
    ...
    private var toolsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            SectionHeader(title: "Tools & Integrations", icon: "wrench.and.screwdriver")
                .opacity(sectionsAppeared ? 1 : 0)
                .offset(x: sectionsAppeared ? 0 : -20)
            ...
            VStack(spacing: 12) {
                SettingsActionRow(
                    ...
                    iconGradient: theme.primaryGradient,
                    title: "Tools",
                    subtitle: "Pick tools to power your chat"
                ) {
                    showingTools = true
                }
                ...
                SettingsActionRow(
                    ...
                    title: "Configure MCP",
                    subtitle: "Manage tool providers"
                ) {
                    showingMCP = true
                }
                ...
                SettingsActionRow(
                    ...
                    title: "Birth Details",
                    subtitle: "Personalize astrology features"
                ) {
                ...
                SettingsActionRow(
                    ...
                    subtitle: "View & manage scheduled sends"
                ) {
                    showingScheduledEmails = true
                }
