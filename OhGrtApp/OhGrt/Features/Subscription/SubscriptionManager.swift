...
class SubscriptionManager: ObservableObject {
    ...
    /// Usage limits based on current tier
    var usageLimits: UsageLimits {
        UsageLimits(tier: currentTier)
    }

    /// Product identifiers for subscriptions
    private let productIdentifiers = [
        PricingConfig.monthlyProductId,  // Plus tier
        PricingConfig.yearlyProductId    // Pro tier
    ]
    ...
    /// Update the current tier based on subscription status
    private func updateCurrentTier() {
        switch subscriptionStatus {
        case .subscribed(let productId, _):
            if productId == PricingConfig.yearlyProductId {
                currentTier = .pro
            } else if productId == PricingConfig.monthlyProductId {
                currentTier = .plus
            } else {
                currentTier = .plus // Default to plus for unknown product
            }
    ...
    func hasAccess(to feature: PremiumFeature) -> Bool {
        let requiredTier = feature.minimumTier
        return tierLevel(currentTier) >= tierLevel(requiredTier)
    }

    /// Get tier level for comparison
    private func tierLevel(_ tier: PremiumTier) -> Int {
        switch tier {
        case .free: return 0
        case .plus: return 1
        case .pro: return 2
    ...
    func requiresUpgrade(for feature: PremiumFeature) -> Bool {
        return !hasAccess(to: feature)
    }

    /// Get required tier for a feature
    func requiredTier(for feature: PremiumFeature) -> PremiumTier {
        return feature.minimumTier
    }

    /// Check if current tier is premium (Plus or Pro)
    var isPremium: Bool {
        currentTier != .free
    }
