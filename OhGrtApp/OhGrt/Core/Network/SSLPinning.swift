...
final class SSLPinningDelegate: NSObject, URLSessionDelegate {
    ...
    private var usesEmbeddedCert = false
    ...
    override init() {
        self.pinnedHost = AppConfig.shared.apiBaseURL.host ?? "d23.ai"
        super.init()
    ...
    private func loadPinnedCertificates() {
        // Try to load .cer file for the current host
        if let certPath = Bundle.main.path(forResource: pinnedHost, ofType: "cer"),
           let certData = try? Data(contentsOf: URL(fileURLWithPath: certPath)) {
            pinnedCertificates.append(certData)
            print("[SSLPinning] Loaded certificate from bundle for \(pinnedHost)")
        }

        // Try to load .der file as fallback
        if let derPath = Bundle.main.path(forResource: pinnedHost, ofType: "der"),
           let derData = try? Data(contentsOf: URL(fileURLWithPath: derPath)) {
            pinnedCertificates.append(derData)
            print("[SSLPinning] Loaded DER certificate from bundle for \(pinnedHost)")
        }

        // Fallback: use embedded base64 cert only for legacy host
        if pinnedCertificates.isEmpty,
           pinnedHost == "api.d23.ai",
           let embeddedData = Data(base64Encoded: Self.embeddedCertBase64) {
            pinnedCertificates.append(embeddedData)
            usesEmbeddedCert = true
            print("[SSLPinning] Loaded embedded certificate")
        }

        if pinnedCertificates.isEmpty {
            print("[SSLPinning] WARNING: No pinned certificates found in bundle for \(pinnedHost)")
        }
    }
