...
final class ThemeManager: ObservableObject {
    ...
    private enum StorageKeys {
        static let colorScheme = "theme_color_scheme"
        static let accentTheme = "theme_accent_theme"
    }

    private func loadStoredTheme() {
        let defaults = UserDefaults.standard
        if let rawTheme = defaults.string(forKey: StorageKeys.accentTheme),
           let storedTheme = AccentTheme(rawValue: rawTheme) {
            accentTheme = storedTheme
        }

        if let rawScheme = defaults.string(forKey: StorageKeys.colorScheme) {
            switch rawScheme {
            case "light":
                colorScheme = .light
            case "dark":
    ...
    private func saveColorScheme() {
        ...
        UserDefaults.standard.set(value, forKey: StorageKeys.colorScheme)
    }

    private func saveAccentTheme() {
        UserDefaults.standard.set(accentTheme.rawValue, forKey: StorageKeys.accentTheme)
    }
    ...
    /// App-wide background view for the active theme
    @ViewBuilder
    var appBackground: some View {
        if isLightMode {
            ...
            ZStack {
                ...
                RadialGradient(
                    colors: [
                        accentTheme.primary.opacity(0.15),
                        Color.clear
                    ],
                    center: .topLeading,
                    startRadius: 10,
    ...
    var cardBackground: Color {
        isLightMode ? lightCardBackground : Color(red: 0.12, green: 0.1, blue: 0.16)
    }
    var cardBorder: Color { accentTheme.primary.opacity(0.2) }

    // Text colors
    var primaryText: Color { isLightMode ? .black : Color(red: 0.93, green: 0.9, blue: 0.96) }
    var secondaryText: Color { isLightMode ? .secondary : Color(red: 0.7, green: 0.66, blue: 0.78) }
...
extension View {
    ...
    func themedGradientBackground() -> some View {
        self.background(
            ThemeManager.shared.appBackground
        )
    }
...
struct PulseAnimationModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .onAppear {
                withAnimation(ThemeManager.AnimationSettings.pulse) {
                    isPulsing = true
                }
            }
    }
...
struct FloatingOrbs: View {
    ...
    var body: some View {
        ...
        .ignoresSafeArea()
        .onAppear {
            // Single smooth animation instead of repeating
            ...
            withAnimation(.easeOut(duration: 2).delay(0.6)) {
                offset3 = 12
            }
        }
    }
