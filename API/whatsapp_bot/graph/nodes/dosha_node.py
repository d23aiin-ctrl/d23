...
from common.i18n.responses import get_dosha_label
...
async def handle_dosha_check(state: BotState) -> dict:
    ...
    if missing:
        dosha_name = _get_dosha_display_name(specific_dosha, detected_lang)
        missing_details = get_dosha_label("missing_details", detected_lang)

        return {
            "response_text": f"*{dosha_name}*\n\n{missing_details}",
            "response_type": "text",
    ...
    try:
        ...
        if result["success"]:
            return _format_dosha_response(result["data"], specific_dosha, detected_lang)
        else:
            raw_error = result.get("error", "")
            user_message = sanitize_error(raw_error, "dosha check")
            dosha_name = _get_dosha_display_name(specific_dosha, detected_lang)
            error_msg = get_dosha_label("error", detected_lang)
            return {
                "response_text": f"*{dosha_name}*\n\n{error_msg}",
                "response_type": "text",
                "should_fallback": False,
...
def _get_dosha_display_name(dosha_type: str, lang: str = "en") -> str:
    """Get localized display name for dosha type."""
    if dosha_type:
        return get_dosha_label(dosha_type, lang)
    return get_dosha_label("dosha", lang)
...
def _format_dosha_response(data: dict, specific_dosha: str = None, lang: str = "en") -> dict:
    """Format dosha check response (localized)."""
    person = data.get("person", {})
    chart_summary = data.get("chart_summary", {})

    # Get localized labels
    title = get_dosha_label("title", lang)
    dob_label = get_dosha_label("dob", lang)
    birth_place_label = get_dosha_label("birth_place", lang)
    chart_summary_label = get_dosha_label("chart_summary", lang)
    moon_sign_label = get_dosha_label("moon_sign", lang)
    ascendant_label = get_dosha_label("ascendant", lang)
    nakshatra_label = get_dosha_label("nakshatra", lang)
    ...
    response += f"*{dob_label}:* {person.get('dob', '')}\n"
    response += f"*{birth_place_label}:* {person.get('birth_place', '')}\n\n"
    ...
    consult = get_dosha_label("consult", lang)
    response += f"\n_{consult}_"
