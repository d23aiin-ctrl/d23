"""DashScope-based implementation of VisionRepository."""
...
class DashScopeVisionRepository(VisionRepository):
    """VisionRepository implementation using DashScope (Qwen VL) API."""
    ...
    async def extract_text(self, image_base64: str, language: str = "en") -> ImageAnalysisResult:
        ...
        extracted = ExtractedText(
            text=text,
            confidence=0.9 if text and "no text" not in text.lower() else 0.0,
            language=language,
        )
        ...
        if text:
            ...
            for line in lines:
                ...
                if name and len(name) > 1:
                    objects.append(DetectedObject(name=name, confidence=0.8))
    ...
    def _parse_document_response(self, text: str) -> DocumentInfo:
        """Parse structured document response."""
        doc = DocumentInfo()
        doc.raw_text = text
    ...
    def _parse_receipt_response(self, text: str) -> ReceiptInfo:
        """Parse structured receipt response."""
        receipt = ReceiptInfo()
    ...
    def _parse_receipt_item(self, line: str) -> dict | None:
        """Parse a receipt line item."""
        # Format: - [item name] x[qty] @ [price]
    ...
    def _parse_food_response(self, text: str) -> FoodInfo:
        """Parse structured food identification response."""
        food = FoodInfo()
