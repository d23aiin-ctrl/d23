...
from common.stores.pending_location_store import get_pending_location_store
...
async def classify_domain(state: BotState) -> dict:
    """
    Classify the user query into a domain.

    This is the first routing step that determines which
    sub-graph should handle the query.

    Args:
        state: Current bot state with whatsapp_message

    Returns:
        Updated state with domain field
    """
    ...
    return {
        "domain": best_domain,
        "current_query": state.get("whatsapp_message", {}).get("text", ""),
    }
