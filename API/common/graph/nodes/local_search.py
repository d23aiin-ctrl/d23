...
async def handle_local_search(state: BotState) -> dict:
    ...
    2. "Near me" search - asks for location, then searches
    ...
    logger.info(f"handle_local_search called: phone={phone}, message_type={message_type}, location_data={location_data}, is_web={is_web_request}")
        ...
        if pending:
            ...
            try:
                ...
                await whatsapp_client.send_text_message(
                    to=phone,
                    text=f"🔍 Searching for *{search_query}* near your location...\nPlease wait a moment."
                )
            except Exception as e:
    ...
    if has_indicator and not location:
        ...
        # Save pending search (WhatsApp only)
        ...
        response_text = (
            f"To find *{search_term}* near you, please share your location.\n\n"
            "Or specify a city like \"{} in Delhi\"".format(search_term)
        )
        return {
    ...
    if search_query and not location:
        ...
        response_text = (
            f"To find *{search_query}* near you, please share your location.\n\n"
            "Or specify a city like \"{} in Delhi\"".format(search_query)
        )
        return {
            "response_text": response_text,
            "response_type": RESPONSE_TYPE_LOCATION_REQUEST,
    ...
    if not query:
        response_text = (
            ...
            "*Example:* Hospitals in Dwarka New Delhi"
        )
        return {
            "response_text": response_text,
            "response_type": "text",
    ...
    return await _execute_search(query, search_query or original_query)
        ...
        if result["success"]:
            ...
            if places:
                ...
                for i, place in enumerate(places[:5], 1):
                    ...
                    # Title with rating on same line
                    rating_str = f" ⭐{rating}" if rating else ""
                    reviews_str = f"({reviews})" if reviews else ""
                    response_lines.append(f"{i}. *{title}*{rating_str} {reviews_str}")

                    # Distance + Address on one line (concise)
                    if distance_text and place_address:
                        # Shorten address if too long
                        short_addr = place_address[:50] + "..." if len(place_address) > 50 else place_address
                        response_lines.append(f"   📍 {distance_text} • {short_addr}")
                    elif place_address:
                        short_addr = place_address[:60] + "..." if len(place_address) > 60 else place_address
                        response_lines.append(f"   📍 {short_addr}")

                    # Phone only (no long maps URL)
                    if phone:
                ...
                # Short footer
                response_lines.append("_\"more\" for more results_")
                ...
                response_text = (
                    f"❌ *No {search_query}* found within {radius_km}km of {location_name}*\n\n"
                    f"Try:\n"
                    f"• Searching for something else\n"
                    f"• A different category (e.g., 'restaurant', 'shop', 'atm')"
                )
...
async def _execute_search(query: str, display_query: str) -> dict:
    """
    ...
    try:
        ...
        if result["success"]:
            ...
            if places:
                # Clean, concise format
                response_lines = [f"📍 *{display_query.title()}*\n"]
                ...
                for i, place in enumerate(places[:5], 1):
                    ...
                    # Distance + Address on one line (concise)
                    if distance_text and address:
                        short_addr = address[:50] + "..." if len(address) > 50 else address
                        response_lines.append(f"   📍 {distance_text} • {short_addr}")
                    elif address:
                        short_addr = address[:60] + "..." if len(address) > 60 else address
                        response_lines.append(f"   📍 {short_addr}")
                ...
                response_text = f"I couldn't find places for '{display_query}'. Try being more specific or check the spelling."
            ...
            response_text = (
                ...
                f"{user_message}\n\n"
                "*Try:*\n"
                "- Be more specific with the location\n"
                "- Check spelling of place names\n"
                "- Use format: 'restaurants near Connaught Place Delhi'"
            )
            return {
                "tool_result": result,
                "response_text": response_text,
